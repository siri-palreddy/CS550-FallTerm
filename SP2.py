...
playerLevel = 1 #This is where the player must choose their level, difficulty increasing with level.
...
class Game:
    ...
    def __init__(self, master): #Creating main function: creating the mine grid, with most of the images.
        ...
        self.lostImage = ImageTk.PhotoImage(file="LOST.png")
        #self.wonImage = PhotoImage(file="WIN.png")
        self.wonImage = ImageTk.PhotoImage(file="WIN.png")
        #self.normalImage = PhotoImage(file="NORMAL.png")
        self.normalImage = ImageTk.PhotoImage(file="NORMAL.png")
        self.createButtons(self.mineFrame)
        

                
    def createButtons(self, master): #Creating the buttons of the game.
        self.mineBoardButtons = [[0 for x in range(totalColumns)] for x in range(totalRows)]
        for i in range(len(mineBoard)):
            for j in range(len(mineBoard[i])): #Creating main buttons, actions when clicked.
        ...
        self.statusBtn.config(image=self.normalImage,width="25",height="25") #The images placed when a mine is clicked, or you lose.  

    def rightClick(self,event,m, k): #Function for flagging bombs with right click.
        # Use globAl counter and not initiate a local variable called mineCounter
        global mineCounter
        btnVal = mineBoard[m][k]
        #If this is a mine then disable the mine
        if (btnVal == '*'): #or (btnVal == 0) or (btnVal == 1) or (btnVal == 2): #Button functions if mine is flagged
    ...
    def setButtonText(self, m, k):
        ...
        self.mineBoardButtons[m][k].config(highlightbackground=BGCOLOR)
