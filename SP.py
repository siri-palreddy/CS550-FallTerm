#EC: Write a program that allows you to play a 5 x 5 game of tic-tac-toe against the computer
#When a player gets three in a row, game over. Uses AI.
...
class Game:
    ...
    def createButtons(self, master):
        ...
        for i in range(len(mineBoard)):
            for j in range(len(mineBoard[i])):
                ...
                b = Button(master, bg='#818a2a',width=5, padx=0, pady=10,command=lambda iLocal = i,jLocal = j: self.buttonClick(iLocal,jLocal))
    ...
    def selectSafeButtons(self, m, k):
        if (m > 0):
            ...
            self.mineBoardButtons[m-1][k].config(state='disabled')
            #self.selectSafeButtons(m-1, k)
            if (k>0):
                self.setButtonText(m-1, k-1)
                self.mineBoardButtons[m-1][k-1].config(state='disabled')
                #self.selectSafeButtons(m-1, k-1)
            if(k< (totalColumns-1)):
                self.setButtonText(m-1, k+1)
                self.mineBoardButtons[m-1][k+1].config(state='disabled')
                #self.selectSafeButtons(m-1, k+1)
        ...
        if (k>0):
            ...
            self.mineBoardButtons[m][k-1].config(state='disabled')
           # self.selectSafeButtons(m, k-1)
        if(k< (totalColumns-1)):
            self.setButtonText(m, k+1)
            self.mineBoardButtons[m][k+1].config(state='disabled')
            #self.selectSafeButtons(m, k+1)
